...
const CsFeature7 = () => {
  ...
  const [soimpletation, setSoImpletation] = useState(false);
  const [pclcompliance, setPclCompliance] = useState(false);
  ...
  useEffect(() => {
    const unsubscribe = onSnapshot(collection(db, 'cyber-security'), (snapshot) => {
      ...
      setItems(itemsArray.slice(6, 7)); // Show only the first 5 items
    });
      ...
      <div className="relative bg-[#072966] mx-4 md:mx-8 lg:mx-20 mb-5 rounded-2xl">
        <div className="flex flex-col md:flex-row justify-center items-center h-full">
          ...
          <div className="w-full md:w-1/3 text-white text-left mx-4 md:mx-10 sm:px-5">
            ...
            <div className={` z-[29] ${toggle ? "" : "hidden"} absolute top-20 left-0 md:left-20 flex justify-center items-center`}>
              <div className="bg-[#072966] shadow-md shadow-slate-400 rounded-2xl h-[50%] m-2 md:w-[70%] md:p-4 p-2">
                ...
                <div className="text-center">
                  <p className="text-2xl">Compliance and Regulatory Services</p>
                  <p className="mt-3">
                    Compliance and regulatory services in cybersecurity ensure that organizations meet legal requirementsand mitigate risks efficiently.</p>
                </div>
                <div className="border border-dashed mt-2"></div>
                <div>
                  <p className="pt-5 text-xl">Compliance and Regulatory Services</p>
                  <p className="mt-3">
            ...
            <br />
            {/* book */}
            <div className={`z-[29] ${consultaion ? "" : "hidden"} absolute top-[3rem] left-0 md:left-[23rem] flex justify-center items-center`} onClick={() => setConsultaion(false)}>
              <div className="bg-[#072966] shadow-md shadow-slate-400 rounded-2xl h-[50%] md:w-[100%] md:p-4 p-2 m-2" onClick={(e) => e.stopPropagation()}>
                <div className="flex justify-end">
                  ...
                  <div className="block ">
                    <div class="mt-2">
                      <input class="form-check-input h-4 w-4" type="checkbox" value=""
                        checked={gdpr} onChange={(e) => setGdpr(e.target.checked)} id="id" />
                      <label class="form-check-label ms-2" for="gdpr">
                        GDPR Compliance
                      </label>
                    </div>
                    <div class="form-check">
                      <input class="form-check-input h-4 w-4" type="checkbox" value=""
                        checked={soimpletation}
                        onChange={(e) => setSoImpletation(e.target.checked)}
                        id="soimpletation" />
                      <label class="form-check-label ms-2" for="id2">
                        SO 27001 Implementation
                      </label>
                    </div>
                    <div class="form-check">
                      <input class="form-check-input h-4 w-4" type="checkbox" value=""
                        checked={pclcompliance}
                        onChange={(e) => setPclCompliance(e.target.checked)}
                        id="id3" />
