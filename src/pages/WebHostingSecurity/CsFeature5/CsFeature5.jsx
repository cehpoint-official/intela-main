...
const CsFeature5 = () => {
  ...
  const [securityArchitecture, setSecurityArchitecture] = useState(false);
  const [secureDevelopment, setsecureDevelopment] = useState(false);
  const [secureNetwork, setSecureNetwork] = useState(false);
      ...
      <div className="relative bg-[#072966] mx-4 md:mx-8 lg:mx-20 mb-5 rounded-2xl">
        <div className="flex flex-col md:flex-row justify-center items-center h-full">
          ...
          <div className="w-full md:w-1/3 text-white text-left mx-4 md:mx-10 sm:px-5">
            <h2 className="text-xl md:text-2xl lg:text-3xl mx-3 mb-3 md:mb-5 mt-3">
              Security Architecture and Design
            </h2>
            <p
              className="text-sm md:text-base mx-3 lg:text-lg pt-7 mb-5"
            ...
            <div className={` z-[29] ${toggle ? "" : "hidden"} absolute top-20 left-0 md:left-20 flex justify-center items-center`}>
              <div className="bg-[#072966] shadow-md shadow-slate-400 rounded-2xl h-[50%] m-2 md:w-[70%] md:p-4 p-2">
                ...
                <div className="text-center">
                  <p className="text-2xl">Security Architecture and Design </p>
                  <p className="mt-3">
                    Security architecture and design in cybersecurity focus on creating resilient systems with robust defenses to safeguard against threats and ensure data security.</p>
                </div>
                <div className="border border-dashed mt-2"></div>
                <div>
                  <p className="pt-5 text-xl">Security Architecture and Design </p>
                  <p className="mt-3">
                    {viewmore ? (
                      viewmore.map(item => (
                        // Ensure that `item.paragraph1` matches the field name in your Firestore documents
                        <div key={item.id}>
                          <h2>{item.Architectureviewmore}</h2>
                        </div>
                      ))
                    ) : (
                      <span>Loading...</span>
            ...
            <div className={`z-[29] ${consultaion ? "" : "hidden"} absolute top-[5rem] left-0 md:left-[25rem] flex justify-center items-center`} onClick={() => setConsultaion(false)}>
              <div className="bg-[#072966] shadow-md shadow-slate-400 rounded-2xl h-[50%] md:w-[200%] md:p-4 p-2 m-2" onClick={(e) => e.stopPropagation()}>
                <div className="flex justify-end">
                  ...
                  <div className="block ">
                    <div class="mt-2">
                      <input class="form-check-input h-4 w-4" type="checkbox" value=""
                        checked={securityArchitecture}
                        onChange={(e) => setSecurityArchitecture(e.target.checked)}
                    ...
                    <div class="form-check">
                      <input
                        checked={secureDevelopment}
                        onChange={(e) => setsecureDevelopment(e.target.checked)}
                        class="form-check-input h-4 w-4" type="checkbox" value="" id="secureDevelopment" />
                    ...
                    <div class="form-check">
                      <input
                        checked={secureNetwork}
                        onChange={(e) => setSecureNetwork(e.target.checked)}
                        class="form-check-input h-4 w-4" type="checkbox" value="" id="id3" />
