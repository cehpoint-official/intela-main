...
const CsFeature6 = () => {
  ...
  const [disterRecovery, setDisterRecovery] = useState(false);
  ...
  useEffect(() => {
    const unsubscribe = onSnapshot(collection(db, 'cyber-security'), (snapshot) => {
      ...
      setItems(itemsArray.slice(5, 6)); // Show only the first 5 items
    });
      ...
      <div className="relative bg-[#072966] mx-4 md:mx-8 lg:mx-20 rounded-2xl">
        <div className="flex flex-col md:flex-row justify-around items-center h-full">
          <div className="w-full md:w-1/3 text-white text-left mx-4 md:mx-8 sm:px-5">
            <h2 className="text-xl md:text-2xl mx-3 lg:text-3xl  mb-3 md:mb-5 mt-3">
              Security Risk Assessment and Management
            </h2>
            <p
              className="text-sm md:text-base lg:text-lg mx-3 mb-5 pt-7"
            ...
            >
              ...
              {/* Security Risk Assessment and Management involves identifying, analyzing, and mitigating security risks to protect assets and data. It helps organizations understand threats and vulnerabilities to make informed decisions about security measures... */}
            </p>
            <button onClick={e => {
              e.stopPropagation()
              setToggle(!toggle)
            ...
            <div className={` z-[29] ${toggle ? "" : "hidden"} absolute top-20 left-0 md:left-20 flex justify-center items-center`}>
              <div className="bg-[#072966] shadow-md shadow-slate-400 rounded-2xl h-[50%] m-2 md:w-[70%] md:p-4 p-2">
                ...
                <div className="text-center">
                  <p className="text-2xl">Security Risk Assessment and Management </p>
                  <p className="mt-3">Security risk assessment and management in cybersecurity entails identifying and maintain continuous security.</p>
                </div>
                <div className="border border-dashed mt-2"></div>
                <div>
                  <p className="pt-5 text-xl">Security Risk Assessment and Management </p>
                  <p className="mt-3 ">
                    {viewmore ? (
                      viewmore.map(item => (
                        // Ensure that `item.paragraph1` matches the field name in your Firestore documents
                        <div key={item.id}>
                          <h2>{item.RiskAssessmentviewmore}</h2>
                        </div>
                      ))
                    ) : (
                      <span>Loading...</span>
            ...
            <br />
            {/* book */}
            <div className={`z-[29] ${consultaion ? "" : "hidden"} absolute top-[5rem] left-0 md:left-[23rem] flex justify-center items-center`} onClick={() => setConsultaion(false)}>
              <div className="bg-[#072966] shadow-md shadow-slate-400 rounded-2xl h-[50%] md:w-[70%] md:p-4 p-2 m-2" onClick={(e) => e.stopPropagation()}>
                ...
                <div className='flex mt-5 justify-between'>
                  ...
                  <input value={user.Name}
                    onChange={e => setUser({ ...user, Name: e.target.value })} class="border border-gray-300 text-black h-8 rounded-sm text-sm" type="search" />
                </div>
                <div className='flex mt-5 justify-between'>
                  <p className='mt-[1%] font-semibold'>Contact number:</p>
                  <input value={user.Contactnumber}
                    onChange={e => setUser({ ...user, Contactnumber: e.target.value })} class="border border-gray-300 text-black h-8 rounded-sm text-sm" type="search" />
                </div>
                <div className='flex mt-5 justify-between'>
                  <p className='mt-[1%] font-semibold'>Email:</p>
                  <input value={user.Email}
                    onChange={e => setUser({ ...user, Email: e.target.value })} class="border border-gray-300 text-black h-8 rounded-sm text-sm " type="search" />
                </div>
                <div className='flex mt-5 justify-between'>
                  <p className='mt-[1%] font-semibold'>Meeting Date:</p>
                  <input value={user.MeetingDate}
                    onChange={e => setUser({ ...user, MeetingDate: e.target.value })} class="border border-gray-300 text-black w-[10.2rem] rounded-sm text-sm" type="date" />
                </div>
                <div className=''>
                  <div className=''>
                    <p value={user.Choose}
                  ...
                  <div className="block ">
                    <div class="mt-2">
                      <input class="form-check-input h-4 w-4" type="checkbox" value="" id="riskAssessment"
                    ...
                    <div class="form-check">
                      <input class="form-check-input h-4 w-4" type="checkbox" value="" id="businessContinuity"
                        checked={businessContinuity}
                        onChange={(e) => setBusinessContinuity(e.target.checked)}
                      />
                    ...
                    <div class="form-check">
                      <input class="form-check-input h-4 w-4" type="checkbox" value="" id="disterRecovery"
                        checked={disterRecovery}
                        onChange={(e) => setDisterRecovery(e.target.checked)}
                      />
