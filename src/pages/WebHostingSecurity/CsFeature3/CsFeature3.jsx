...
const CsFeature3 = () => {
  ...
  const [incidentResponse, setIncidentResponse] = useState(false);
  ...
  useEffect(() => {
    const unsubscribe = onSnapshot(collection(db, 'cyber-security'), (snapshot) => {
      ...
      setItems(itemsArray.slice(2, 3)); // Show only the first 5 items
    });
      ...
      <div className="relative bg-[#072966] mx-4 md:mx-8 lg:mx-20 mb-5 rounded-2xl">
        <div className="flex flex-col md:flex-row justify-center items-center h-full">
          ...
          <div className="w-full md:w-1/3 text-white text-left mx-4 md:mx-10 sm:px-5">
            ...
            <p
              className="text-sm md:text-base lg:text-lg pt-7 mb-5 mx-3"
            ...
            <div className={` z-[29] ${toggle ? "" : "hidden"} absolute top-20 left-0 md:left-20 flex justify-center items-center`}>
              <div className="bg-[#072966] shadow-md shadow-slate-400 rounded-2xl m-2 h-[50%] md:w-[50%] md:p-4 p-2">
                <div className="flex justify-end">
                  <button onClick={handleClick}><i class="bi bi-x-lg"></i></button>
                </div>
                <div className="text-center">
                  <p className="text-2xl">Security Incident Response and Management</p>
                  <p className="mt-3">Cybersecurity incident response involves swiftly addressing security breaches to minimize damage and restore normalcy.</p>
                </div>
                <div className="border border-dashed mt-2"></div>
                <div>
                  <p className="pt-5 text-xl">Security Incident Response and Management</p>
                  <p className="mt-3">
                    {viewmore ? (
                      viewmore.map(item => (
                        // Ensure that `item.paragraph1` matches the field name in your Firestore documents
                        <div key={item.id}>
                          <h2>{item.IncidentResponsevirwmore}</h2>
                        </div>
                      ))
                    ) : (
                      <span>Loading...</span>
            ...
            <br />
            {/* book */}
            <div className={`z-[29] ${consultaion ? "" : "hidden"} absolute top-[5rem] left-0 md:left-[20rem] flex justify-center items-center`} onClick={() => setConsultaion(false)}>
                ...
                <div className=''>
                  ...
                  <div className="block ">
                    ...
                    <div class="form-check">
                      <input class="form-check-input h-4 w-4" type="checkbox" value="" id="incidentResponse"
                        checked={incidentResponse}
                        onChange={(e) => setIncidentResponse(e.target.checked)}
                      />
                    ...
                    <div class="form-check">
                      <input class="form-check-input h-4 w-4" type="checkbox" value=""
                        checked={forensic}
                        onChange={(e) => setForensic(e.target.checked)}
                        id="forensic" />
