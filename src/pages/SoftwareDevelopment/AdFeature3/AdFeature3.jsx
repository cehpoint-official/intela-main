      ...
      <div className="relative bg-[#072966] mx-4 md:mx-8 lg:mx-20 mb-5 rounded-2xl">
        <div className="flex flex-col md:flex-row justify-center items-center h-full">
          ...
          <div className="w-full md:w-1/3 text-white text-left mx-4 md:mx-10 sm:px-5">
            ...
            <p className="text-sm md:text-base mx-4 lg:text-lg pt-7 mb-5" style={{ fontFamily: "Roboto" }}>
              {items ? (
                items.map(item => (
                  // Ensure that `item.paragraph1` matches the field name in your Firestore documents
                  <div key={item.id}>
            ...
            <button onClick={e => {
              e.stopPropagation()
              setToggle(!toggle)
            }} className="mx-4 text-[#BD981B] underline" href="">
              View More
            </button>
            {/* pop_up */}
            <div className={` z-[29] ${toggle ? "" : "hidden"} absolute top-40 left-0 md:left-20 flex justify-center items-center`}>
              <div className="bg-[#072966] shadow-md shadow-slate-400 rounded-2xl h-[50%] md:w-[60%] md:p-6 p-2">
                ...
                <div className="text-center">
                  <p className="text-2xl">IOS App Development</p>
                  <p className="mt-3">iOS app development is crafting applications for Apple devices.</p>
                </div>
                <div className="border border-dashed mt-2"></div>
                <div>
                  <p className="pt-5 text-xl">IOS App Development</p>
                  <p className="mt-5 ">
                    {/* iOS app development involves programming applications for Apple's mobile operating system using Swift or Objective-C and the iOS SDK.. */}
                    {viewmore ? (
                      viewmore.map(item => (
                        // Ensure that `item.paragraph1` matches the field name in your Firestore documents
                        <div key={item.id}>
                          <h2>{item.iosviewmore}</h2>
                        </div>
                      ))
                    ) : (
                      <span>Loading...</span>
                    ...
                    <p>
                      {viewmore ? (
                        viewmore.map(item => (
                          // Ensure that `item.paragraph1` matches the field name in your Firestore documents
                          <div key={item.id}>
                            <h2>{item.consultationnumber2}</h2>
                          </div>
                        ))
                      ) : (
                        <span>Loading...</span>
